...
def crawl(url,usuario,password,output_filename,crawl_limit=0,log=False,log_filename='none',crawl_depth=0):
	...
	try:
		printout('\n[+]TARGET URL OR WEBSITE: '+url,output_filename)
		
		if output_filename:
			printout('\n[+]OUPUT FILE: '+output_filename.name,output_filename)
		if log:
			printout('\n[+]COMMON LOG OUTPUT FILE: '+log_filename.name,output_filename)

		printout('',output_filename)
		printout('[+]CRAWLING LINKS',output_filename)
			...
			try:
				...
				if response:
					...
					if not isinstance(response, int):
						...
						printout('\n[+] '+error_codes[str(response)],output_filename)
						if log:
		...
		printout('\n[+]TOTAL URL CRAWLED: '+str(len(links_crawled)),output_filename)
		printout('',output_filename)
...
def external_links(root_url,external_vector,output_filename):
	...
	try:
		...
		printout('',output_filename)
		printout('\n[+]SUBDOMAINS FOUND: ',output_filename)
		tmp=[]
		...
		printout('\n[+]TOTAL : '+str(len(tmp)),output_filename)
       
		printout('',output_filename)
		printout('\n[+]EMAILS FOUND: ',output_filename)
		for link in external_vector:
			if 'mailto' in urlparse.urlparse(link).scheme:
				printout('   [-] '+link.split(':')[1].split('?')[0],output_filename)
        
		printout('',output_filename)
		printout('\n[+]EXTERNAL SOURCES AND REDIRECTIONS: ',output_filename)
		for link in external_vector:
			parsed = urlparse.urlparse(link)
			if parsed.netloc:
...
def indexing_search(usuario, password,links_vector,output_filename):
	...
	try:
		...
		printout('\n[+]DIRECTORIES FOUND:',output_filename)
		for directory in directories:
			printout('[+]'+directory,output_filename)
		printout('[+]TOTAL DIRECTORIES FOUND: '+str(len(directories)),output_filename)
		printout('',output_filename)

		printout('\n[+]DIRECTRY WITH INDEX',output_filename)
		dots='.'
		for directory in directories:
			sys.stdout.flush()
			...
			try:
				...
				if response:
					...
					if response==1:
						continue
					if response==0:
						print '\n[-]SKYPPING THE REST OF DIRECTORIES'
						break
		...
		printout('\n[+]TOTAL DIRECTRIES WITH INDEX: '+str(len(indexing)),output_filename)
		printout('',output_filename)
		...
		if len(files_vector)>0:
			...
			if export_file_list:
				try:
					...
					printout('[+]EXPORTING THE FILES: '+output_name.rpartition('.')[0]+'.files',output_filename)
				except OSError,error:
					if 'File exists' in error:
							printout('[+] EXPORTING TO: '+output_name.rpartition('.')[0]+'.files',output_filename)
							pass
					else:
	...
	try:
		if len(files_vector)>0:
			...
			if interactive_flag:
				...
				print '[+]THE FOLLOWINF FILES FOUND WHILE CRAWLING:'
				print '   ',
				print '\n[+]THE EXTENSION FOUND ARE  ',extensions_found
				extensions_to_download= raw_input('\n[+]ENTER EXTENSION OF FILES THAT YOU WANT TO DOWNLOAD Ex.: png,pdf,css.')
			...
			if ( len(list_of_files_to_download) > 0 ):
				...
				printout('',output_filename)
				printout('[+]DOWNLOADING FILES: '+extensions_to_download,output_filename)
				...
				try:
					...
					printout('[+]FILES WILL BE SAVED AT: '+output_directory,output_filename)
				except OSError, error:
					if 'File exists' in error:
						print '\n[-]DIRECTORY ALREADY EXISTS PRESS a TO OVERWRITE'
						try:
							raw_input()
							printout('\n[+]LOG WILL BE SAVED AT: '+output_directory,output_filename)
						except KeyboardInterrupt:
							printout('\n[-]DOWNLOAD FILES ABORTED',output_filename)
							return 1 
					else:
						printout('\n[-]DOWNLOAD ABORT. Error while creating output directory.',output_filename)
						...
						if not isinstance(response, int):
							...
							try:
								...
								if 'File exists' in error:
									...
									printout('CAN NOT CREAT FOLDER TO SAVE FILE '+output_directory+'/'+i.rpartition('/')[2],output_filename)
...
def statistics(global_time, directories, indexing, links_crawled, files, extensions_found, output_filename):
	...
	try:
		...
		if len(links_crawled) > 1:
			...
			print
			if output_filename:
				print '\n[+]OUTPUT STORED: {0}'.format(os.path.realpath(output_name))
				print
			print '[+]TOTAL TIME TAKEN: {0} seconds ({1} min)'.format(round(global_time,2),round((global_time/60),2))
			print '[+]AVG TIME PER REQUEST: {0} seconds'.format(round(avg_time_per_query,2))
			print
...
def main():
	...
	try:
		...
		if check_url(url_to_crawl):
			...
			if save_output:
				output_name = urlparse.urlparse(url_to_crawl).netloc+'.txt'
				try:
